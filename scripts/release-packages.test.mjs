...
test("generates one deterministic, isolated three-package candidate", {
  ...
  assert.equal(enginePackage.dependencies["@piaoxianguo/miniagent-core"], "0.8.0");
  assert.equal(extensionsPackage.dependencies["@piaoxianguo/miniagent-core"], "0.8.0");
...
test("accepts only the exact frozen candidate package policy", async () => {
  ...
  const cases = [
    ["changed archive", async (root) => {
      await appendFile(path.join(root, "archives", "piaoxianguo-miniagent-core-0.8.0.tgz"), "changed");
    }, /integrity/i],
    ...
    ["wrong dependency", async (root) => {
      const packagePath = path.join(root, "packages", "engine", "package.json");
      const packageJson = JSON.parse(await readFile(packagePath, "utf8"));
      packageJson.dependencies["@piaoxianguo/miniagent-core"] = "^0.8.0";
      await writeFile(packagePath, `${JSON.stringify(packageJson, null, 2)}\n`);
    }, /dependencies/i],
    ["horizontal MiniAgent dependency", async (root) => {
      const packagePath = path.join(root, "packages", "engine", "package.json");
      const packageJson = JSON.parse(await readFile(packagePath, "utf8"));
      packageJson.dependencies["@piaoxianguo/miniagent-extensions"] = "0.8.0";
      await writeFile(packagePath, `${JSON.stringify(packageJson, null, 2)}\n`);
    }, /dependencies/i],
    ...
    ["symlinked archive escapes the candidate", async (root) => {
      const archivePath = path.join(root, "archives", "piaoxianguo-miniagent-core-0.8.0.tgz");
      await rm(archivePath);
      await symlink(path.join(candidateRoot, "archives", "piaoxianguo-miniagent-core-0.8.0.tgz"), archivePath);
    }, /symlink|canonical|contain/i],
  ];
...
test("documents only the split 0.8 consumer routes in English and Chinese", async () => {
...
if (args[0] === "install") {
  if (args.slice(-2).join(" ") !== "@piaoxianguo/miniagent-engine@0.8.0 @piaoxianguo/miniagent-extensions@0.8.0") process.exit(97);
  const coreRoot = path.join(process.cwd(), "node_modules", "@piaoxianguo", "miniagent-core");
  mkdirSync(coreRoot, { recursive: true });
  writeFileSync(path.join(coreRoot, "package.json"), JSON.stringify({ name: "@piaoxianguo/miniagent-core", version: "0.8.0" }));
  process.exit(0);
}
if (args.join(" ") === "ls --all --json") {
  process.stdout.write(JSON.stringify({ dependencies: {
    "@piaoxianguo/miniagent-engine": { dependencies: { "@piaoxianguo/miniagent-core": { version: "0.8.0" } } },
    "@piaoxianguo/miniagent-extensions": { dependencies: { "@piaoxianguo/miniagent-core": { version: "0.8.0" } } },
  } }));
  process.exit(0);
}
process.exit(98);
...
test("rejects a perturbed accepted tarball before publication planning", async () => {
  ...
  try {
    await cp(candidateRoot, copiedCandidate, { recursive: true });
    await appendFile(path.join(copiedCandidate, "archives", "piaoxianguo-miniagent-core-0.8.0.tgz"), "perturbed");
    const result = spawnSync(process.execPath, [
      "scripts/publish-release-packages.mjs",
...
test("waits for core visibility and resolves ambiguous publication by identity", async () => {
  ...
  try {
    ...
    assert.match(lines[0], /miniagent-core-0\.8\.0\.tgz --provenance --access public --registry https:\/\/registry\.npmjs\.org\/$/);
    assert.match(lines[1], /miniagent-engine-0\.8\.0\.tgz --provenance --access public --registry https:\/\/registry\.npmjs\.org\/$/);
    assert.match(lines[2], /miniagent-extensions-0\.8\.0\.tgz --provenance --access public --registry https:\/\/registry\.npmjs\.org\/$/);
  } finally {
    await rm(temporaryRoot, { recursive: true, force: true });
  }
});

test("allows only the unique complete-history addition of the exact 0.8 release intent", async () => {
  ...
  try {
    const markerPath = path.join(temporaryRoot, "scripts", "release-intent-0.8.0.json");
    ...
    execFileSync("git", ["add", "scripts/release-intent-0.8.0.json"], { cwd: temporaryRoot });
    ...
    await writeFile(markerPath, exactMarker.replace('"0.8.0"', '"0.8.1"'));
    execFileSync("git", ["add", "scripts/release-intent-0.8.0.json"], { cwd: temporaryRoot });
    ...
    await writeFile(path.join(unavailableRoot, "scripts", "release-intent-0.8.0.json"), exactMarker);
...
test("workflow retains and reaccepts the candidate before serialized publication", async () => {
  ...
  assert.deepEqual(publish.concurrency, { group: "miniagent-npm-0.8.0", "cancel-in-progress": false });
  ...
  assert.match(workflowText, /miniagent-0\.8\.0-candidate-\$\{\{ github\.sha \}\}-\$\{\{ github\.run_id \}\}/i);

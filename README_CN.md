...
- 新增命令：`/git [status|log]`、`/diff [--staged] [path]`、`/editor [initial text]`、`/diagnostics`、`/activity`、`/permissions`、`/system`、`/references`、`/init [--force]`；headless 支持 `--list-references [--json]`、`--list-snapshots [--json]`、`--restore-snapshot <turnId> [--json]`、`--reapply-snapshot <turnId> [--json]`、`--show-permissions [--json]` 和 `--show-system-prompt [--json]`。

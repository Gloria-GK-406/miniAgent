...
export class GLMEngine implements LLMEngine, ModelCatalogLLMEngine {
  readonly name = "glm";
  ...
  streamGenerate(
    ...
    if (Array.isArray(requestOrMessages)) {
      if (!this.config || !this.client || !tools) {
        throw new Error(
          "GLMEngine legacy streamGenerate requires constructor config",
        );
      }
    ...
    const client = new OpenAI({
      ...
      baseURL: requestOrMessages.provider.baseUrl || GLM_BASE_URL,
    });
    return this.streamWithClient(
      client,

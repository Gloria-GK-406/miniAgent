...
describe("CLI entry args", () => {
  ...
  it("prints runtime status without opening the TUI", () => {
    expect(parseCLIEntryArgs(["--status"])).toEqual({
      type: "status",
    });
    expect(parseCLIEntryArgs([
      "--cwd",
  ...
  it("prints workspace overview without opening the TUI", () => {
    expect(parseCLIEntryArgs(["--overview"])).toEqual({
      type: "overview",
    });
    expect(parseCLIEntryArgs([
      "--cwd",
  ...
  it("lists workspace snapshots without opening the TUI", () => {
    ...
    expect(parseCLIEntryArgs(["--cwd", "C:/repo", "--session", "s2", "--list-snapshots", "--json"])).toEqual({
      type: "list-snapshots",
      cwd: "C:/repo",
      sessionId: "s2",
      output: "json",
  ...
  it("shows system prompt without opening the TUI", () => {
    expect(parseCLIEntryArgs(["--show-system-prompt"])).toEqual({
      type: "show-system-prompt",
    });
    expect(parseCLIEntryArgs(["--cwd", "C:/repo", "--agent", "plan", "--show-system-prompt", "--json"])).toEqual({
      type: "show-system-prompt",
      agent: "plan",
      cwd: "C:/repo",
      output: "json",
  ...
  it("rejects prompted status mode", () => {
    expect(parseCLIEntryArgs(["--status", "prompt"])).toEqual({
      type: "error",
      message: "Unexpected prompt for --status",
    });
    expect(parseCLIEntryArgs(["--status", "--list-sessions"])).toEqual({
      type: "error",
      message: "Cannot combine --status with another headless mode",
    });
  });

  it("rejects prompted overview mode", () => {
    expect(parseCLIEntryArgs(["--overview", "prompt"])).toEqual({
      type: "error",
      message: "Unexpected prompt for --overview",
    });
    expect(parseCLIEntryArgs(["--status", "--overview"])).toEqual({
      type: "error",
      message: "Cannot combine --status with another headless mode",
    });
  });
  ...
  it("rejects prompted snapshot listing mode", () => {
    expect(parseCLIEntryArgs(["--list-snapshots", "prompt"])).toEqual({
      type: "error",
      message: "Unexpected prompt for --list-snapshots",
    });
    expect(parseCLIEntryArgs(["--status", "--list-snapshots"])).toEqual({
      type: "error",
      message: "Cannot combine --status with another headless mode",
    });
  });
  ...
  it("rejects malformed system prompt update options", () => {
    ...
    expect(parseCLIEntryArgs(["--show-system-prompt", "prompt"])).toEqual({
      type: "error",
      message: "Unexpected prompt for --show-system-prompt",
    });
  });
  ...
  it("formats concise help text", () => {
    ...
    expect(help).toContain("--show-system-prompt");
